...
impl SolanaIndexer {
    ...
    async fn process_transactions(&self) -> Result<(), AppError> {
        ...
        println!("Processing slots {} to {}", last_slot + 1, current_slot);
    ...
    async fn process_slot_range(&self, start_slot: u64, end_slot: u64) -> Result<(), AppError> {
        ...
        let blocks = self.solana_client.get_blocks_with_commitment(
            start_slot,
            Some(end_slot),
        ...
        for slot in blocks {
            if let Err(e) = self.process_block(slot).await {
                eprintln!("Error processing block {}: {:?}", slot, e);
                continue;
            }
        }
    ...
    async fn process_block(&self, slot: u64) -> Result<(), AppError> {
        let block = self.solana_client.get_block_with_config(
            slot,
    ...
    async fn process_transaction(
        ...
        if let Some(meta) = &tx_with_meta.meta {
            let block_time = tx_with_meta
                .block_time
                .map(|t| {
                    chrono::DateTime::from_timestamp(t, 0).unwrap_or_else(|| chrono::Utc::now())
                })
                .unwrap_or_else(|| chrono::Utc::now());

            // Process different instruction types
            // self.process_marketplace_instruction(&signature, &transaction, meta, block_time)
            //     .await?;
        }
